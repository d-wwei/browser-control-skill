...
**macOS**: element index (inject indexing script, then `window.__interactiveElements[i]` — most reliable) → text match → CSS selector → coordinates (last resort)
...
| Navigate | `osascript -e 'tell application "Google Chrome" to tell front window to make new tab with properties {URL:"URL"}'` |
...
- **Never navigate in the user's current tab** — always open a new tab (`make new tab with properties {URL:...}`) or a dedicated task window. The user's existing tabs must be preserved.
...
- If `front window` throws `-1719` (hidden Chrome window), use JXA to find a visible window: `osascript -l JavaScript -e 'function run(){var c=Application("Google Chrome");var w=c.windows();for(var i=0;i<w.length;i++){if(w[i].visible())return c.execute(w[i].activeTab(),{javascript:"document.title"});}return"No visible window"}'`
